...
export class TableDataService {
  private dataSubj = new BehaviorSubject<Entity[]>(TABLE_DATA);
  ...
  filterByName = (name: string) => {
    ...
    if (name === '') {
      this.dataSubj.next(TABLE_DATA);

      return;
    }

    const filteredData: Entity[] = [];

    TABLE_DATA.forEach((entity: Entity) => {
      this.filterItemsByName(name, entity, filteredData);
    });

    this.dataSubj.next(filteredData);
  };

  private filterItemsByName(name: string, entity: Entity, result: Entity[]): void {
    ...
    if (entity.children) {
      entity.children.forEach((child: any) => this.filterItemsByName(name, child, result));
    }
  }
}
